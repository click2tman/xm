 ...
 */
function apigee_responsive_autodesk_preprocess_html(&$vars) {
 ...
 */
function apigee_responsive_autodesk_preprocess_page(&$vars) {
  global $user;
  ...
  $dropdown_links = array(
      ...
      array(
          'classes' => array('glyphicon', 'glyphicon-list'),
          'text' => t('My Workbench'),
          'url' => '/admin/workbench',
      ),
      array(
  ...
  $dropdown_invitedeveloper = array(
      array(
          'classes' => array('glyphicon', 'glyphicon-send'),
          'text' => t('Invite Developer'),
          'url' => '/admin/workbench/my-developers',
      ),
  );
  ...
  // Check if the user has the 'Partner Admin' role, when 'Partner Admin' has role id 11
  if (array_key_exists(11, $user->roles)) {
    // Add Invite developer to dropdown
    array_splice( $dropdown_links, 1, 0, $dropdown_invitedeveloper );
  }

  drupal_alter('apigee_responsive_autodesk_links', $dropdown_links);
  ...
  // Add app - Moving the products field to the top 
  if($vars['page']['content']['system_main']['#type'] == 'form' && $vars['page']['content']['system_main']['#form_id'] == 'devconnect_developer_apps_edit_form'){
    $vars['page']['content']['system_main']['api_product']['#weight'] = 0;
    $vars['page']['content']['system_main']['api_product']['#title'] = 'Select Product';
    $vars['page']['content']['system_main']['api_product']['#title_display'] = 'invisible';
    $vars['page']['content']['system_main']['api_product']['#prefix'] = '<h3>Select Product</h3>';
    $vars['page']['content']['system_main']['name']['#prefix'] = '<h3>Basic Information</h3>';
  }
 ...
 */
function apigee_responsive_autodesk_preprocess_region(&$vars) {
 ...
 */
function apigee_responsive_autodesk_preprocess_block(&$vars) {
 ...
 * @return string
 */
function _apigee_responsive_autodesk_status_label_callback($status, $pull_right = FALSE) {
  if ($status == 'Revoked') {
 ...
 */
function apigee_responsive_autodesk_preprocess_devconnect_developer_apps_list(&$vars) {
 ...
 */
function apigee_responsive_autodesk_preprocess_bootstrap_modal_forms(&$vars) {
 ...
 */
function apigee_responsive_autodesk_preprocess_node(&$vars) {
 ...
 */
function apigee_responsive_autodesk_form_alter(&$form, &$form_state, $form_id) {
  switch ($form_id) {
    case 'user_login':
 ...
 */
function apigee_responsive_autodesk_form_devconnect_monetization_plan_form_alter(&$form, &$form_state) {
  if (isset($form['comparisons'])) {
    // Add comparison template javascript
    $form['#attached']['js'][] = drupal_get_path('theme', 'apigee_responsive_autodesk') . '/js/monetization/devconnect_monetization_plan_detail_comparison.js';
  }
}
 ...
 *   The form state.
 */
function apigee_responsive_autodesk_form_devconnect_monetization_company_details_form_alter(&$form, &$form_state) {

  $form['#attached']['js'][] = drupal_get_path('theme', 'apigee_responsive_autodesk') . '/js/monetization/jquery.select-to-autocomplete.js';
...
function apigee_responsive_autodesk_form_devconnect_monetization_company_bank_details_form_alter(&$form, &$form_state) {
 ...
 *   The form state.
 */
function apigee_responsive_autodesk_form_apigee_company_company_form_alter(&$form, &$form_state) {
    apigee_responsive_autodesk_form_devconnect_monetization_company_details_form_alter($form, $form_state);
}

/**
 * Implements theme_developer_app_tabs().
 */
function apigee_responsive_autodesk_developer_app_tabs(&$vars, $apps = NULL) {
 ...
 */
function apigee_responsive_autodesk_preprocess_user_profile(&$vars) {
 ...
 * Implements hook_preprocess().
 */
function apigee_responsive_autodesk_preprocess(&$vars, $hook) {
 ...
 */
function apigee_responsive_autodesk_advanced_forum_l(&$vars) {
  $text = $vars['text'];
 ...
 */
function apigee_responsive_autodesk_developer_app_panes(&$vars) {
 ...
 */
function apigee_responsive_autodesk_menu_link($vars) {
  ...
  if ($element['#below']) {
    if (!(($element['#original_link']['menu_name'] == 'management') && (module_exists('navbar')))) {
      ...
      if ($element['#below']) {
        ...
        $below = _apigee_responsive_autodesk_get_below($element['#below']);
      }
...
function _apigee_responsive_autodesk_get_below($element) {
  $output = '';
  foreach ($element as $mlid => $item) {
    if (is_numeric($mlid)) {
      if ($item['#below']) {
        ...
        if ($item['#below']) {
          $below = _apigee_responsive_autodesk_get_below($item['#below']);
        }
        $output .= $below . "</li>\n";
      }
      else {
 ...
 */
function apigee_responsive_autodesk_menu_tree__primary(&$vars) {
 ...
 * Menu Local Tasks
 */
function apigee_responsive_autodesk_menu_local_tasks(&$vars) {
  $output = '';
 ...
 */
function apigee_responsive_autodesk_advanced_forum_reply_link(&$vars) {
  $node = $vars['node'];
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function apigee_responsive_autodesk_form_search_form_alter(&$form, &$form_state) {
  ...
  if ($default_search == 'site') {
    ...
    unset($form['basic']);
    unset($form['action']);
    $form['#submit'] = array('_apigee_responsive_autodesk_search_form_submit');
  }
}
 ...
 * @param $form_state
 */
function _apigee_responsive_autodesk_search_form_submit($form, &$form_state) {
...
function apigee_responsive_autodesk_app_status($app) {
 ...
 */
function apigee_responsive_autodesk_preprocess_maintenance_page(&$vars) {
  ...
  apigee_responsive_autodesk_preprocess_html($vars);
}
 ...
 */
function apigee_responsive_autodesk_apigee_company_switcher($variables) {
