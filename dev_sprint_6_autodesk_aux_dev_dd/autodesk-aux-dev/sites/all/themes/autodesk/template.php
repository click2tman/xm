 ...
 */
function autodesk_preprocess_html(&$vars) {
  ...
  if (module_exists('devconnect_monetization')) {
    require_once drupal_get_path('theme', 'autodesk') . '/templates/monetization/template.php';
    drupal_add_css(drupal_get_path('theme', 'autodesk') . '/templates/monetization/css/monetization.css');
  }
 ...
 */
function autodesk_preprocess_page(&$vars) {
  global $user;
  ...
  drupal_alter('autodesk_links', $dropdown_links);
  ...
  if($vars['page']['content']['system_main']['#type'] == 'form' && $vars['page']['content']['system_main']['#form_id'] == 'devconnect_developer_apps_edit_form'){
	...
	$vars['page']['content']['system_main']['actions']['cancel'] = array(
		'#type' => 'button',
        '#value' => t('Cancel'),
		'#attributes' => array('onClick' => "window.location.href = '/user/me/apps'; return true;"),
		'#weight' => 99
	);
  }  
   /* print '<pre>';
 ...
 */
function autodesk_preprocess_region(&$vars) {
 ...
 */
function autodesk_preprocess_block(&$vars) {
 ...
 * @return string
 */
function _autodesk_status_label_callback($status, $pull_right = FALSE) {
  if ($status == 'Revoked') {
 ...
 */
function autodesk_preprocess_devconnect_developer_apps_list(&$vars) {
 ...
 */
function autodesk_preprocess_bootstrap_modal_forms(&$vars) {
 ...
 */
function autodesk_preprocess_node(&$vars) {
 ...
 */
function autodesk_form_alter(&$form, &$form_state, $form_id) {
  switch ($form_id) {
    case 'user_login':
    ...
    case 'webform_client_form_6':
     $form['actions']['submit']['#attributes'] = array('class' => array('go'));
  }
}
 ...
 */
function autodesk_form_devconnect_monetization_plan_form_alter(&$form, &$form_state) {
  if (isset($form['comparisons'])) {
    // Add comparison template javascript
    $form['#attached']['js'][] = drupal_get_path('theme', 'autodesk') . '/js/monetization/devconnect_monetization_plan_detail_comparison.js';
  }
}
 ...
 *   The form state.
 */
function autodesk_form_devconnect_monetization_company_details_form_alter(&$form, &$form_state) {

  $form['#attached']['js'][] = drupal_get_path('theme', 'autodesk') . '/js/monetization/jquery.select-to-autocomplete.js';
...
function autodesk_form_devconnect_monetization_company_bank_details_form_alter(&$form, &$form_state) {
 ...
 *   The form state.
 */
function autodesk_form_apigee_company_company_form_alter(&$form, &$form_state) {
    autodesk_form_devconnect_monetization_company_details_form_alter($form, $form_state);
}

/**
 * Implements theme_developer_app_tabs().
 */
function autodesk_developer_app_tabs(&$vars, $apps = NULL) {
 ...
 */
function autodesk_preprocess_user_profile(&$vars) {
 ...
 * Implements hook_preprocess().
 */
function autodesk_preprocess(&$vars, $hook) {
 ...
 */
function autodesk_advanced_forum_l(&$vars) {
  $text = $vars['text'];
 ...
 */
function autodesk_developer_app_panes(&$vars) {
 ...
 */
function autodesk_menu_link($vars) {
  ...
  if ($element['#below']) {
    if (!(($element['#original_link']['menu_name'] == 'management') && (module_exists('navbar')))) {
      ...
      if ($element['#below']) {
        ...
        $below = _autodesk_get_below($element['#below']);
      }
...
function _autodesk_get_below($element) {
  $output = '';
  foreach ($element as $mlid => $item) {
    if (is_numeric($mlid)) {
      if ($item['#below']) {
        ...
        if ($item['#below']) {
          $below = _autodesk_get_below($item['#below']);
        }
        $output .= $below . "</li>\n";
      }
      else {
 ...
 */
function autodesk_menu_tree__primary(&$vars) {
 ...
 * Menu Local Tasks
 */
function autodesk_menu_local_tasks(&$vars) {
  $output = '';
 ...
 */
function autodesk_advanced_forum_reply_link(&$vars) {
  $node = $vars['node'];
 ...
 * Implements hook_form_FORM_ID_alter().
 */
function autodesk_form_search_form_alter(&$form, &$form_state) {
  ...
  if ($default_search == 'site') {
    ...
    unset($form['basic']);
    unset($form['action']);
    $form['#submit'] = array('_autodesk_search_form_submit');
  }
}
 ...
 * @param $form_state
 */
function _autodesk_search_form_submit($form, &$form_state) {
...
function autodesk_app_status($app) {
 ...
 */
function autodesk_preprocess_maintenance_page(&$vars) {
 ...
 */
function autodesk_apigee_company_switcher($variables) {
  drupal_add_css(drupal_get_path('theme', 'autodesk') . '/css/company-switcher.css');
  $output_links = '';
